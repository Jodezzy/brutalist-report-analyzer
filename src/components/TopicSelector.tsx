...
import { Search, Info, Filter, Calendar, FolderOpen } from "lucide-react";
...
const TopicSelector: React.FC<TopicSelectorProps> = ({ selectedTopic, onSelectTopic, timeRange, onSelectTimeRange, onStartAnalysis, isLoading, loadFromFile }) => {
	...
	return (
		<div className="flex flex-col gap-6 md:gap-8">
						...
						<div className="flex flex-wrap gap-2">
							<button
								...
								className={`px-3 py-2 rounded-lg flex items-center transition-all ${
									selectedTopic === null ? "bg-indigo-600 text-white shadow-md" : "bg-gray-700 text-gray-300 hover:bg-gray-600 hover:text-white"
								}`}
							>
								🔍 All Topics
							</button>
							{TOPICS.map((topic) => (
								<button
									...
									className={`px-3 py-2 rounded-lg flex items-center transition-all ${
										selectedTopic === topic ? "bg-indigo-600 text-white shadow-md" : "bg-gray-700 text-gray-300 hover:bg-gray-600 hover:text-white"
									}`}
								>
						...
						<div className="flex justify-between items-center">
							<button
								...
								className={`flex-1 py-3 px-4 rounded-lg flex items-center justify-center transition-all ${
									timeRange === "today" ? "bg-indigo-600 text-white shadow-md" : "bg-gray-700 text-gray-300 hover:bg-gray-600 hover:text-white"
								}`}
							>
							...
							<button
								...
								className={`flex-1 py-3 px-4 rounded-lg flex items-center justify-center transition-all ${
									timeRange === "last-week" ? "bg-indigo-600 text-white shadow-md" : "bg-gray-700 text-gray-300 hover:bg-gray-600 hover:text-white"
								}`}
							>
								<span className="text-lg mr-2">📆</span>
								Last Week
            ...
            <Button onClick={onStartAnalysis} disabled={isLoading} className="w-full md:w-auto shadow-md hover:shadow-lg transition-shadow">
              <Search className="w-4 h-4 mr-2" />
              {isLoading ? "Analyzing..." : "Start Analysis"}
            </Button>
            <div className="px-2 text-gray-500">or</div>
            <div className="group relative">
						<Button onClick={loadFromFile} disabled={isLoading} variant="outline" size="sm">
							<FolderOpen className="h-4 w-4 mr-2" />
							Open JSON
						</Button>
						<div className="absolute bottom-full mb-2 right-0 bg-gray-800 text-xs text-white p-2 rounded shadow-md opacity-0 group-hover:opacity-100 transition-opacity w-48 pointer-events-none z-10">
